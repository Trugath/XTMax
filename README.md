...
This fork is a personal project aimed at using XTMax with my IBM 5155 (Portable PC); bring-up notes, timing defaults, and harness targets often reflect that machine, even though much of the stack applies to other PC/XT-class systems too.

Unless a subdirectory or file says otherwise, this fork is distributed under the MIT license in [LICENSE](./LICENSE), which lists copyright holders (including upstream authors) and paths that use other licenses. Some bundled components keep their own original licensing, for example [software/sd/LICENSE.TXT](./software/sd/LICENSE.TXT).
