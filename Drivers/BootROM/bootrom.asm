...
; Use XT-compatible geometry. Very large head counts (eg 255) can confuse
; older DOS/FDISK/FORMAT flows and cause partition type/format mismatches.
%define NUM_HEADS           (16)
...
fixed_disk_parameters_table:
    dw  (NUM_CYLINDERS - 1)             ; cylinders (last cylinder reserved)
    db  NUM_HEADS                       ; heads
    db  0, 0                            ; reserved
...
;      FL = <TRASH>
;
; Full 32-bit: (cyl * NUM_HEADS + head) * SECTORS_PER_TRACK + (sector - 1).
; The previous version popped DX after cyl*heads MUL, discarding the high word
; when cyl * NUM_HEADS exceeded 65535 (large virtual geometries).
;
compute_lba:
    push bp
    mov bp, sp
    ...
    mov cx, [bp-4]      ; original CX (cylinder/sector encoding)
    xor ax, ax
    mov al, cl
    and al, 0xc0
    shl ax, 1
    shl ax, 1
    mov al, ch          ; cylinder (10 bits) in AX

    mov cx, NUM_HEADS
    mul cx              ; DX:AX = cylinder * NUM_HEADS

    mov cl, [bp-1]      ; DH from saved DX (head)
    xor ch, ch
    add ax, cx
    adc dx, 0           ; DX:AX = cylinder * NUM_HEADS + head

    mov si, ax
    mov di, dx          ; DI:SI = multiplicand M

    mov cx, SECTORS_PER_TRACK
    mov ax, di
    mul cx              ; M_hi * SPT -> DX:AX (DX = 0 for valid M)
    push ax             ; addend for high word of M_lo * SPT

    mov ax, si
    mul cx              ; DX:AX = M_lo * SPT
    pop si
    add dx, si          ; DX:AX = M * SPT
...
;
; Busy-wait delays for PC/XT (no INT 15h AH=86h). Tuned for ~4.77 MHz 8086;
; runs slightly long on faster CPUs (safe for SD power-up / CMD0 retry).
;
delay_100ms_xt:
    push cx
    mov cx, 25000
