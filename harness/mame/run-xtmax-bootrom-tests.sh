...
XTMAX_MAME_EXPECT_CS="52736" \
XTMAX_MAME_EXPECT_HALT="1" \
XTMAX_MAME_ASSERT_SCRIPT="${XTMAX_HARNESS_ROOT}/lua/assert_cpu_state.lua" \
XTMAX_MAME_ASSERT_STARTUP_WAIT="${XTMAX_MAME_INVALID_ASSERT_STARTUP_WAIT:-70}" \
XTMAX_MAME_ASSERT_TIMEOUT="${XTMAX_MAME_INVALID_ASSERT_TIMEOUT:-10}" \
./harness/mame/run-xtmax-device-tests.sh
