...
build_xtmax_test_boot_image "${menu_image}"

echo "Running XTMax ROM service menu path."
XTMAX_MAME_SD_IMAGE="${menu_image}" \
XTMAX_MAME_ASSERT_SCRIPT="${XTMAX_HARNESS_ROOT}/lua/post_and_assert.lua" \
XTMAX_MAME_EXPECT_TEXT="F/S/D/C|OK" \
XTMAX_MAME_POST_WHEN_TEXT="X?" \
XTMAX_MAME_POST_CODED="xd" \
...
echo "XTMax ROM service menu test passed."
